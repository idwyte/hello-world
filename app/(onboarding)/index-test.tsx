// Obsidian Kinetic: 04–09 · The v2 six-step assessment battery.
// Figma nodes 34:62 (Strength) · 35:72 (Stamina) · 34:91 (Repetitions) ·
// 35:91 (Speed) · 35:110 (Control) · 36:95 (Release).
// Copy: docs/hone-assessment-copy-deck.md (verbatim). Logic:
// docs/hone-assessment-logic-v2.md (PERFECT-scheme approximation).
//
// One route, six internal steps. Replaces the old 2-test battery; the
// legacy Hone Index keeps working via v2ToLegacyMeasurements (fast
// count ×2 ≈ 30 s pulses, endurance straight across).
//
// Step 6 (Release) is the safety-critical screen — its output drives
// the down-training archetype routing in lib/assessment-v2.ts.
...
export default function AssessmentBattery() {
  ...
  function finish() {
    ...
    setV2('strengthOxford', strength);
    setV2('enduranceSeconds', holdSeconds);
    setV2('repCeiling', reps);
    setV2('fastCount', fastCount);
    setV2('coordinationFlags', coordinationNone ? [] : coordination);
    setV2('release', release);
    setV2('symptomFlags', symptoms);
    // Legacy Hone Index continuity — composite + level from the two
    // overlapping measures, so trend charts span the v1→v2 transition.
    const legacy = v2ToLegacyMeasurements({
      fastCount,
      enduranceSeconds: holdSeconds,
    });
    setIndex(scoreIndex(legacy));
